...
public:
    ...
    vector<string> heapSort(const vector<pair<string, double>>& distances);
    //Of the sorting algorithms that we have learned, quick sort is considered the fastest.
    vector<string> quickSort(vector<pair<string, double>>& distances);
    void quickSort(vector<pair<string, double>>& distances, int low, int high);
    void setK(int k);
    int getK();
};
